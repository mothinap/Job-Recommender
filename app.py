...
def get_key_skills(job_title):
    # Read the CSV file into a DataFrame
    df = pd.read_csv('jobs_info.csv')

    # Debug print to see the contents of the DataFrame
    print(df.head())

    # Filter the DataFrame for the specific job title
    filtered_jobs = df[df['Job Title'] == job_title]

    # Debug print to see the filtered DataFrame
    print(filtered_jobs)

    # Check if any jobs were found
    if filtered_jobs.empty:
        print("No jobs found for the title:", job_title)
        return None

    # Get the key skills for the first matching job posting
    key_skills = filtered_jobs['Key Skills'].tolist()
...
def getjob():
    if request.method == 'POST':
        job_name = request.form.get('getjob')
        if not job_name:
            return render_template('get_job.html', error='missing_fields')

        # Retrieve key skills for the entered job title
        skills = get_key_skills(job_name)
        if skills is None:
            return render_template('get_job.html', error='getjob_not_found')

        return render_template('skills.html', job_title=job_name, skills=skills)

    return render_template('get_job.html', error='invalid_request')


def get_job_titles(key_skills):
    # Read the CSV file into a DataFrame
    df = pd.read_csv('jobs_info.csv')

    # Filter the DataFrame for job titles that contain all the specified key skills
    filtered_jobs = df[df['Key Skills'].apply(lambda x: all(skill in x for skill in key_skills))]

    # Get the job titles from the filtered DataFrame
    job_titles = filtered_jobs['Job Title'].tolist()
...
def getskills():
    if request.method == 'POST':
        ...
        if not key_skills:
            return render_template('get_skills.html', error='missing_fields')

        # Split the input key skills string into a list
        key_skills_list = [skill.strip() for skill in key_skills.split(',')]

        # Retrieve job titles that match the entered key skills
        job_titles = get_job_titles(key_skills_list)
        if not job_titles:
            return render_template('get_skills.html', error='no_matching_jobs', key_skills=key_skills)

        return render_template('jobs.html', key_skills=key_skills, job_titles=job_titles)

    return render_template('get_skills.html', error='invalid_request')
...
def recommend_career(data, preferences):
    # Placeholder logic for career recommendation
    return "Software Engineer"  # Example recommendation

def read_csv(file_path):
    # Placeholder logic for reading CSV file
...
def career():
    if request.method == 'POST':
        # Read the CSV file
        file_path = 'Student_Responces.csv'
        data = read_csv(file_path)

        # Get user preferences from the form
        preferences = {
            'Gender': request.form['gender'],
            'Are you confident in your chosen career path?': request.form['confidence'],
            'What factors affect your career decision-making process?': request.form['factors'],
            'How did you choose your career path?': request.form['choice'],
            # Add more preferences here based on your form fields
        }

        # Recommend a career based on preferences
        recommended_career = recommend_career(data, preferences)
        if recommended_career:
            return render_template('result.html', recommended_career=recommended_career)
        else:
            return render_template('result.html', error_message="Sorry, we couldn't find a suitable career based on your preferences.")
...
def display_jobs():
    # Retrieve job data from somewhere (e.g., database or API)
    jobs = retrieve_job_data()  # Example function to retrieve job data

    return render_template('get_job.html', jobs=jobs)
